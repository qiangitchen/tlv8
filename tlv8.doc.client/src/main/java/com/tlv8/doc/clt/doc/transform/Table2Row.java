...
public class Table2Row {
	public Element transform(Docinfo docinfo, TransformConfig paramTransformConfig) throws ModelException {
		if (Utils.isNull(paramTransformConfig))
			paramTransformConfig = new TransformConfig();
		Element localElement1 = DocumentHelper.createElement(TransformConstants.ROWS_QNAME);
		...
		if ((Utils.isNotEmptyString(str1)) && (Utils.isNotEmptyString(str2))) {
			localElement2 = DocumentHelper.createElement(TransformConstants.ROW_QNAME);
			localElement2.addAttribute(TransformConstants.ID_QNAME, str1);
			localElement2.addElement(!paramTransformConfig.isCellNameByRelation() ? "cell"
					: getFirstColumnName(docinfo, paramTransformConfig)).setText(str2);
			localElement1.add(localElement2);
		}
		if (Utils.isNotNull(docinfo)) {
			if (Utils.isNotEmptyString(paramTransformConfig.getRowsConfig().getTreeParent()))
				localElement1.addAttribute("parent", paramTransformConfig.getRowsConfig().getTreeParent());
			if (Utils.isNotEmptyString(paramTransformConfig.getRowsConfig().getTreeSpreadParentRelation()))
				localElement1.addAttribute("tree-parent-relation",
						paramTransformConfig.getRowsConfig().getTreeSpreadParentRelation());
			a(docinfo, paramTransformConfig);
			if (TransformType.ROW_LIST.equals(paramTransformConfig.getRowsConfig().getDataType()))
				b(docinfo, paramTransformConfig, localElement2);
			else if (TransformType.ROW_TREE.equals(paramTransformConfig.getRowsConfig().getDataType()))
	...
	private void c(Docinfo docinfo, TransformConfig paramTransformConfig, Element localElement2) {
		// TODO Auto-generated method stub
	...
	private Element a(String paramTable, TransformConfig paramTransformConfig) {
		Utils.checkNotEmptyString("User data name", paramTable, ResponseTransform.logger);
	...
	public static String getFirstColumnName(Docinfo docinfo, TransformConfig paramTransformConfig)
			throws ModelException {
		if (Utils.isNotEmptyString(paramTransformConfig.getRowsConfig().getSequence()))
			return paramTransformConfig.getRowsConfig().getSequence().split(",")[0];
		...
		if (((Iterator) localObject).hasNext()) {
			ColumnMetaData localColumnMetaData = (ColumnMetaData) ((Iterator) localObject).next();
	...
	private Element a(Docinfo paramRow, TransformConfig paramTransformConfig) throws ModelException {
		...
		if ((paramTransformConfig.isTransformIdColumn()) && (Utils.isNotEmptyString(str1))
				&& (!localTableMetaData.containsColumn(str1))) {
			String localObject1 = "Table idColumn[" + str1 + "]不存在";
			throw new ModelException((String) localObject1);
		}
		if ((paramTransformConfig.isTransformIdColumn()) && (Utils.isNotEmptyString(str1))) {
			localElement.addAttribute(TransformConstants.ID_QNAME,
					SimpleTransform.transToString(paramRow.getValue(str1)));
	...
	@SuppressWarnings("rawtypes")
	private void b(Docinfo docinfo, TransformConfig paramTransformConfig, Element paramElement) throws ModelException {
	...
	public Attribute transform(Map<String, DocPermission> map, TransformConfig arg0) {
		// TODO Auto-generated method stub
		return null;
	}
