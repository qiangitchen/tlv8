...
//			self.funUploadFile(v);
	...
	funUploadFile : function(file){
		...
		var docPath = this.docPath || "/root";
		formdata.append("upload", file);
		formdata.append("size",file.size);
		formdata.append("dbkey",this.data.dbkay);
		formdata.append("rowid",this.data.rowid);
		formdata.append("tablename",this.data.table);
		formdata.append("cellname",this.cellname);
...
//					edit             : true,  						      // 是否可以编辑文件（裁剪）
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				var type = newStr.split(".")[0].split("").reverse().join("");
				// 处理不同类型文件代表的图标
				var fileImgSrc = cpath+"/comon/zyUpload/zyupload/skins/images/fileType/";
				if(type == "rar"){
...
//								}
				...
				ZYFILE = $.extend(ZYFILE, params, param);
				ZYFILE.init();
			};
