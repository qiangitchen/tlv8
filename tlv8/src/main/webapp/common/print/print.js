...
tlv8.HtmlPrint = {
	...
	getHtml : function(formId) {
		...
		for ( var i = 0; i < formId.length; i++) {
			...
			$("#" + formId[i]).find("select").each(function(){//解决select标签没有带值的问题
				var sval = $(this).val();
			...
			var tmpform = $("#" + formId[i]).clone();
			tmpform.find("td").find("div").each(function(){
				$(this).after("<span id='"+this.id+"' style='"+$(this).attr("style")+";text-align:left;'>"+$(this).html()+"</span>").remove();
			});
			tmpform.find("a").each(function(){
				$(this).after("<span id='"+this.id+"' style='"+$(this).attr("style")+"'>"+$(this).html()+"</span>").remove();
			});
			html += $("<div></div>").append(tmpform).html();
		}
		var self = this;
			...
			$(node)
					...
					.each(
							function() {
								...
								$(this.parentNode).css("word-break","break-all");
		...
		$(node)
				...
				.each(
						function() {
							//this.parentNode.style.cssText = "word-break:break-all;font-size:13px;";
							$(this.parentNode).css("word-break","break-all");
							$(this.parentNode).css("text-align","left");
