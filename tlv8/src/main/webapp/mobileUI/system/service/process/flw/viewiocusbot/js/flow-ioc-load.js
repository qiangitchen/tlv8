...
function Group(id, name) {
	...
	this.setGroupArea = function() {
		...
		var num = this.nodes.length;
		var node = null;
		var minLeft = document.body.clientWidth;
		for (var i = 0; i < num; i++) {
			node = this.nodes[i];
			...
			if(node.left - node.width - this.rightWidth < minLeft){
				minLeft = node.left - node.width - this.rightWidth;
			}
		}
		...
		if ((maxWidth + this.rightWidth + document.body.clientWidth/2) > document.body.clientWidth) {
			obj.style.width = (maxWidth + this.rightWidth + document.body.clientWidth/2) + 'px';
			$(obj).parent().scrollLeft(minLeft+45);//矫正滚动条部分
		} else {
			obj.style.width = '100%';
		}
	};
