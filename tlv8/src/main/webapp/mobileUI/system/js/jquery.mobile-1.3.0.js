...
$.widget( "mobile.checkboxradio", $.mobile.widget, {
	...
	_create: function() {
		var self = this,
			...
			$(input).attr("value",input.prop( "checked" )?"on":"off");
...
//选择复选框
$.checkbox = function(elname){
	$("#"+elname).attr("checked", true);
	var RSp = $("label[for='"+elname+"']")[0];
	RSp.className = "ui-checkbox-on ui-btn ui-btn-corner-all ui-fullsize ui-btn-icon-left ui-first-child ui-btn-up-c";
	var RSpS = RSp.getElementsByTagName("span");
	for(var i=0; i < RSpS.length; i++){
		if(RSpS[i].className.indexOf("ui-icon-checkbox-off")>-1){
			RSpS[i].className = "ui-icon ui-icon-checkbox-on ui-icon-shadow";
		}
	}
}