$(document).ready(
		function() {
			...
			param.set("type", "week");
			var re = tlv8.XMLHttpRequest("getSelfWorkPlanAction", param, "post");
			if (re.data.flag == "true") {
				...
				if (re.count > 0) {
					...
					$("#contenView").html(
							"<div style='color:red;width:100%;padding:10px;text-align:center;'>未查询到您创建过的周工作计划!</div>");
				}
			} else {
				alert("获取周计划信息失败");
			}
...
function newData() {
	window.open("WeekWorkPlan.html","_self");
}
...
function doSearch(name) {
	...
	param.set("type", "week");
	var result = tlv8.XMLHttpRequest("getSelfWorkPlanAction", param,
			"post");
...
function createList(data) {
	...
	for (var i = 0; i < data.length; i++) {
		var title = "周工作计划： " + data[i].STARTTIME + " 至 " + data[i].FINISHTIME;
		...
		html += '</p></div>';
		html += '<div class="mui-table-cell mui-col-xs-6">';
		html += '<p class="mui-h6 mui-ellipsis">创建时间：<strong>' + data[i].TIME
				+ '</strong></p>';
		html += '</div>';
		html += '</div>';
		html += '</div></li>';
...
function openDetail(id) {
	var url = "WeekWorkPlan.html?rowid=" + id;
