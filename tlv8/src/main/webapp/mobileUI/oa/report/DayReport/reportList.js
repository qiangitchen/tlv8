$(document).ready(
		function() {
			...
			param.set("type", "day");
			var re = tlv8.XMLHttpRequest("getSelfReportAction", param, "post");
			if (re.data.flag == "true") {
				...
				if (re.count > 0) {
					...
					$("#contenView").html(
							"<div style='color:red;width:100%;padding:10px;text-align:center;'>未查询到您创建过的日报!</div>");
				}
			} else {
				alert("获取信息失败");
			}
...
function doSearch(name) {
	...
	if (result.data.flag == "true") {
		...
		if (result.count > 0) {
			...
			$("#contenView").html(
					"<font color='red'>未找到与' " + name + " '相关的信息!</font>");
		}
...
function openDetail(id) {
	var url = "writeDayReport.html?sData1=" + id;
	window.open(url, "_self");
}
function newData() {
	window.open('writeDayReport.html','_self');
}
...
function loadMore() {
	...
	var re = tlv8.XMLHttpRequest("getSelfReportAction", param);
	if (re.data.flag == "true") {
		var data = re.data.data;
