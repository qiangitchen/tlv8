$(document).ready(
		function() {
			...
			var re = tlv8.XMLHttpRequest("getSelfReportAction?tmp"
			...
			if (re.data.flag == "true") {
				...
				if (re.count > 0) {
					...
					$("#contenView").html(
							"<div style='color:red;width:100%;padding:10px;text-align:center;'>未查询到您创建过的月报!</div>");
				}
			} else {
				alert("获取信息失败");
			}
...
function openDetail(id) {
	var url = "writeMonthReport.html?sData1=" + id;
	window.open(url, "_self");
}
function newData() {
	var url = "writeMonthReport.html";
	window.open(url, "_self");
}
