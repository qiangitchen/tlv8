...
function initPage() {
	var sql = "select * from wait_task_view where FEDITDATE is not null order by FCREATEDATE desc";
			...
			function(result) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					html += " <h3>" + data[i].FTITLE
							...
							+ "</span></h3>";
					html += "<p><strong>" + data[i].FSUMMARIZE
							+ "</strong></p>";
					html += "</li>";
				}
				$("#list_view").html(html);
...
function viewDeatail(obj) {
	...
	var url = "filltask_detail.html?sData1=" + rowid + "&option=view&mainid="+mainid;
