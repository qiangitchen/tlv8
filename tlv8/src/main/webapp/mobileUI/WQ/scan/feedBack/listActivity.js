...
function initPage() {
	var sql = "select FID,FQUESTION,FCOMMODITYNAME,FCREATEDATE,FCREATORNAME from WQ_QUALITY_FEEDBACK where FCREATORID = '"
...
//							+ data[i].FCOMMODITYNAME
							+ data[i].FCREATEDATE.substring(0,11)
							+ "</span></h3>";
					html += "<p><strong>" + data[i].FQUESTION
							+ "</strong></p>";
					html += "</li>";
				}
				$("#list_view").html(html);
