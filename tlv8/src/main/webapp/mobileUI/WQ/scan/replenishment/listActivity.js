...
function initPage() {
	var sql = "select FID,FSUBBRANCHNAME,FCOMMODITYNAME,FCREATEDATE,FCREATORNAME from wq_replenishment where FCREATORID = '"
	...
	tlv8.sqlQueryActionforJson("wq", sql,
			function(result) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					html += " <h3>" + data[i].FSUBBRANCHNAME+"("+data[i].FCREATORNAME+")"
