...
$.extend($.jpolite, {
	tabs : new Map(),
	getTab : function(tId){
		//return $.jpolite.tabs.get(tId);
	},
	addTab : function(tId, p){
		if($.jpolite.tabs.containsKey(tId)){
			var tab = $.jpolite.tabs.get(tId);
			var index = $('#mainTab').tabs('getTabIndex',tab);
			$('#mainTab').tabs('select', index);
		}else{
			var content = '<div style="width:100%;height:100%;padding:2px;">'
				+ '<iframe scrolling="auto" frameborder="0"  src="' + p.url
			...
			setTimeout(function(){
				$.jpolite.tabs.put(tId,$('#mainTab').tabs('getSelected'));
			},100)
		}
	},
	getTabID : function(){
	...
	removeTab : function(tId){
		if($.jpolite.tabs.containsKey(tId)){
			var index = $('#mainTab').tabs('getTabIndex',$.jpolite.tabs.get(tId));
			$('#mainTab').tabs('close', index);
			$.jpolite.tabs.remove(tId);
		}
	},
	...
	ontabselected : function(title, index){
		if("首页" == title){
			$("iframe.portal_body_frame").each(function(){
