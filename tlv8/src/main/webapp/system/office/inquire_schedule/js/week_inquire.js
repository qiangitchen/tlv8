...
var weekOnload = {
	onloadrun : function() {
		...
		document.getElementById("divTableHeader").innerHTML=weekData.headerhtml;
		getWeekData(setDate);
		setTableHtml();
...
var setTableHeaderDate = function(v){
	...
	for ( var i = 0; i < 7; i++) {
		...
		weeks.innerHTML = "<font color='#FFFFCC'>" + (days.getMonth() + 1)
				+ "月" + days.getDate() + "日" + "<br>" + dayweek + "</font>";
	...
	mNum = mFirstnum.toString() + mSecondnum.toString()
	...
	sNum = sFirstnum.toString() + sSecondnum.toString()
...
var setTableHtml = function(){
	...
	if(weekData.data.length>0){
		for(var i=0;i<weekData.data.length;i++){
			var personID = weekData.data[i].ID,personName = weekData.data[i].NAME;
			var nDays=weekData.data[i].nDays;
			var nDaytxt = "",singleDaytxt = "";
			var tableHtml = "";/*构件表格*/
			tableHtml="<table bgcolor=\"#FFFFFF\" width='100%' border='1' cellpadding='0' cellspacing='1' bordercolor='#0099FF' style=\"font-family: '宋体';font-size:12;\">" +
				"<tr><td rowspan='2' align='center' valign='middle' width='97' style=\"font-family: '宋体';font-size:14;\" id='td"+ personID +"'>"+ personName +"</td>" +
				"<td colspan='7' id='tdnday"+ personID +"'>&nbsp;</td></tr>" +
				"<tr align='center' valign='middle'>" +
				"<td width='170' id='1td"+ personID +"'>&nbsp;</td>" +
				"<td width='170' id='2td"+ personID +"'>&nbsp;</td>" +
				"<td width='170' id='3td"+ personID +"'>&nbsp;</td>" +
				"<td width='170' id='4td"+ personID +"'>&nbsp;</td>" +
				"<td width='170' id='5td"+ personID +"'>&nbsp;</td>" +
				"<td width='170' id='6td"+ personID +"'>&nbsp;</td>" +
				"<td width='170' id='7td"+ personID +"'>&nbsp;</td></tr></table>";
			divHtml = divHtml + tableHtml;
		}
		var divObj = document.getElementById("divWeekSchedule");
		divObj.innerHTML = divObj.innerHTML + divHtml;
	}
}
		/*为表格装载数据*/
...
//			if(!status||status==null||status=="null"){
//				if(nowDatetimeNum>etime){status = "已过期"; statusColor="red"}
//				if(nowDatetimeNum>stime && nowDatetimeNum<etime){status = "进行中";statusColor="#0000FF";}
//				if(nowDatetimeNum<stime){status = "未开始";	statusColor="#B5005A"}
//			}
				...
				var title = Priority+"("+status+")";
					nDaytxt = nDaytxt + "<div title="+ title
						+ " style='position:relative;left:0;width :40%;line-height:1.5;margin-top:1; border:1 solid #CDCDCD;background-color:#F7F7F7;' id='div"
						+ sid + "'>&nbsp;"
						...
						+ "' spriority='"+ priority +"' onclick='message_affairsLooks(this)'>"+ startdate.substring(0, 16) +" - "+ enddate.substring(0, 16) +" " +"<font color="+ statusColor +">"+ caption +"</font></a></div>";
			...
			if(!status||status==null||status=="null"){
				...
				var title = Priority+"("+status+")";
					singleDaytxt = singleDaytxt + "<div title="+ title
						+ " style='line-height:1.5;margin-top:1; border:1 solid #CDCDCD;background-color:#F7F7F7;' id='div"
						+ sid + "'>"
						+ "<a style=\"text-decoration:'none';\" href='javascript:void(0);' " + "sstartdate='"+ startdate +"' senddate='"+ enddate
						+ "' scaption='"+ caption +"' sid='" + sid + "' sstatus='"+ status + "' scontent='" + content
				...
				if (coordinateDay == 0) {coordinateDay = 7;}
				var coordinate = coordinateDay + "td" + personID;
	...
	/*定位跨天DIV*/
var locateDiv = function(){
	for(var i=0;i<weekData.data.length;i++){
		var personID = weekData.data[i].ID,personName = weekData.data[i].NAME;
		var nDays=weekData.data[i].nDays;
		if(nDays.length>0){
			for ( var n = 0; n < nDays.length; n++) {
				try{
					var jq = jQuery.noConflict();
					var sid = nDays[n].ID;
					var startdate = nDays[n].STARTDATE.substring(0, 10);
					var startdateArray = startdate.split("-");
					var enddate = nDays[n].ENDDATE.substring(0, 10);
					var enddateArray = enddate.split("-");
					var nDaysSnum = startdateArray[0].toString() + startdateArray[1].toString() + startdateArray[2].toString();
					var nDaysEnum = enddateArray[0].toString() + enddateArray[1].toString() + enddateArray[2].toString();
					if (nDaysSnum >= mNum) {//开始日期大于等于星期一
						var affairstartdate = new Date(startdateArray[0],startdateArray[1] - 1, startdateArray[2]);//开始日期
						var affairenddate = new Date(enddateArray[0],enddateArray[1] - 1, enddateArray[2]);//结束日期
						var leftmargin = getLeftmargin(affairstartdate.getDay());//得到左边距参数
						jq("#div" + sid).animate( {left : leftmargin + "%"}, "slow");//定位DIV外左边距
						if (nDaysEnum <= sNum) {//结束时间小于等于星期天
							var divwidth = getDIVwidth(affairstartdate.getDay(),affairenddate.getDay());
							jq("#div" + sid).animate( {width : (divwidth) + "%"}, "slow");//DIV宽度
							if (nDaysEnum = sNum){
								jq("#div" + sid).animate( {right : "0"}, "slow");
							}
						}
						if (nDaysEnum > sNum) {//结束时间大于星期天
							var divwidth = getDIVwidth(affairstartdate.getDay(), 0);
							jq("#div" + sid).animate( {	width : (divwidth+1) + "%"}, "slow");//DIV宽度
							jq("#div" + sid).animate( {right : "0"}, "slow");
						}
					}
					if (nDaysSnum < mNum) {//开始日期小于星期一
						var affairstartdate = new Date(startdateArray[0],startdateArray[1] - 1, startdateArray[2]);//开始日期
						var affairenddate = new Date(enddateArray[0],enddateArray[1] - 1, enddateArray[2]);//结束日期
						jq("#div" + sid).animate( {left : "0%"}, "slow");//定位DIV外左边距
						if (nDaysEnum <= sNum) {//结束时间小于等于星期天
							var divwidth = getDIVwidth(1, affairenddate.getDay());
							jq("#div" + sid).animate( {width : divwidth + "%"}, "slow");//DIV宽度
							if (nDaysSnum = sNum){	jq("#div" + sid).animate( {right : "0"}, "slow");
							}
						}
						if (nDaysEnum > sNum) {//结束时间大于星期天
							jq("#div" + sid).animate( {width : "100%"}, "slow");//DIV宽度
							jq("#div" + sid).animate( {right : "0"}, "slow");
						}
					}
				}catch(e){continue;}
...
//		if(!nDays)nDays = [];
		...
		"sa_psnschedule ,sa_opperson "+
		"WHERE sa_psnschedule.SWHOUSER='"+ nowID +
		"' AND SSTARTDATE_AXIS>="+ mNum +
		" AND SSENDDATE_AXIS<="+ sNum +
		" AND sa_psnschedule.SWHOUSER = sa_opperson.SID AND sa_psnschedule.SSTARTDATE_AXIS=sa_psnschedule.SSENDDATE_AXIS " +
		" ORDER BY sa_psnschedule.SSTARTDATE ASC ";
