	...
	getHtml : function(formId) {
		...
		for (var i = 0; i < formId.length; i++) {
			...
			for (var j = 0; j < inputs.length; j++) {
				var input = inputs[j];
				if ($.browser.version == 9 && input.value != null
						&& input.value != "") {
...
//						})
	...
	executePrintout : function(formId, appendHtml, closeWin, isPreview, options) {
		if (!formId) {
			justep.System.showMessage("没有指定target-id");
			return;
		}
		if (!window.Event) {
			throw new Error("打印组件不支持非IE浏览器");
			return;
		}
		var modes = {
			iframe : "iframe",
		...
		writeDoc.close();
		// "+
		// ((window.Event&&isPreview)?"onload='window.parent.frames['print_html_frame'].wb.ExecWB(7,1);'":"")
		// + "
	...
	Popup : function() {
		...
		// windowAttr += ",width=" + settings.popWd + ",height=" +
		// settings.popHt;
	...
	transHtml : function(node) {
				...
				.each(
						function() {
							...
							node
									...
									.html(
											window.frames[this.id].document.frames[0].document.body.innerHTML
													|| "&nbsp;");
						});
		// }
		...
		$(node).find("textarea").each(
				function() {
					this.parentNode.style.cssText = "word-break:break-all;";
					this.parentNode.innerHTML = this.value ? self.replaceAll(
							self._dhx_encoding(("" + this.value)), "\n",
							"<br/>") : "&nbsp;";
				});
