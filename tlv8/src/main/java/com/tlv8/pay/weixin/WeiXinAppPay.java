...
public class WeiXinAppPay {
	...
	public String getTemp() {
		return "appid=" + WeixinPayConfig.mappid + "&body=" + body + "&mch_id=" + WeixinPayConfig.mmch_id
				+ "&nonce_str=" + nonce_str + "&notify_url="
				+ WeixinPayConfig.notify_url + "&out_trade_no=" + out_trade_no
				+ "&product_id=" + product_id 
				+ "&spbill_create_ip=" + spbill_create_ip 
				+ "&total_fee=" + total_fee + "&trade_type=APP";
	}
	...
	public Element getActionResult() throws Exception {
		String params = "<xml> " + "<appid>" + WeixinPayConfig.mappid+ "</appid>" + "<body>"
				+ body + "</body> " + "<mch_id>" + WeixinPayConfig.mmch_id + "</mch_id>"
	...
	public static void main(String[] args) {
		try {
			System.out.println(new WeiXinAppPay().getCodeUrl());
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
