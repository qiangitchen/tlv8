...
public class GetSendMailAction extends ActionSupport {
	...
	public Object execute() throws Exception {
		...
		String sql = "select FID as ID,fState as STATUS," + "FEMAILNAME as TITLE,FSENDPERNAME as NAME,"
				+ "to_char(FSENDTIME,'yyyy-MM-dd HH24:mi') as TIME, " + "e.FCONSIGNEE as ORGNAME "
				+ "from OA_EM_SendEmail  e " + " where e.fSendPerID = '" + personid + "' " + " and (" + filter + ")"
				+ " order by  fState, FSENDTIME desc";

		try {
			String yd = "";
			List yl = DBUtils.execQueryforList("oa",
					"select count(*) as COUNT from (" + sql + ") where STATUS = '已保存'");
			if (yl.size() > 0) {
				Map m = (Map) yl.get(0);
